...
import static io.milton.cloud.server.db.ScheduledEmail.findPossiblyDue;
...
public class TriggerTimer implements Serializable{
    ...
    public static TriggerTimer create(Organisation org, Website website, Profile forUser, Date fireAt, String createdReason, Session session) {
        TriggerTimer tt = new TriggerTimer();
        tt.setOrganisation(org);
        tt.setFireForProfile(forUser);
        tt.setFireAt(fireAt);
    ...
    /**
     * Find up to maxResults of timers due to be processed and lock them, and return
     * their ID's. They should then be processed in seperate transactions
     * 
     * @param now
     * @param timeoutMins
     * @param maxAttempts
    ...
    public static List<TriggerTimer> findPossiblyDue(Date now, int maxResults, int timeoutMins, int maxAttempts, Session session) {
        ...
        crit.add(Restrictions.gt("fireAt", now)); // is due        
        ...
        notTaken.add(Restrictions.gt("processingStartedAt", timeoutTime));
        crit.add(notTaken);
    ...
    /**
     * The date and time that this should fire at. In reality it wont fire
     * at exactly this time, but will be some time afterwards
     * 
     * @return 
     */
