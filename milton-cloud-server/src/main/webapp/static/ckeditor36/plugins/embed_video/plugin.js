		...
		CKEDITOR.dialog.add( 'videoDialog', function( editor )
		{
			return {
				...
				onShow : function()
				{
					...
					initTree('http://dev.video.ettrema.com/', processData, 2);
...
function initTree(fullUrl, func, deep){
	$.ajax( {url:fullUrl+"_DAV/PROPFIND?fields=name,href,iscollection&depth="+deep, dataType:"JSON", success: func });
}
	...
	$.each(e, function(index, test) {
		if ( index == 0 )
		{
			...
			if ( test.iscollection )
			{
				...
				lmtype = test.name.substr(test.name.lastIndexOf('.')+1, test.name.len);
				lurl = 'http://dev.video.ettrema.com' + test.href
				content += '<li id="' + test.href + '" class="jstree-closed" rel="file" mtype="' + lmtype + '" url="' + lurl + '" src="http://dev.video.ettrema.com/skins/Big_Buck_Bunny_Trailer_480x270.png">';
				content += '<ins class="jstree-icon">&nbsp;</ins>';
		...
		.bind("open_node.jstree", function (e, data) {
			if ( data.args[0][0] != undefined && data.args[0][0] !='#' )
			{
				if ( data.args[0][0].getAttribute('rel')=='folder' && data.args[0].children().find('li').text() == '' )
				{
					...
					initTree('http://dev.video.ettrema.com'+data.args[0][0].id, attachNode, 1);
				} 
			}
...
function onAddFolder()
{
	if ( initialSelect )
	{
		...
		liObj.setAttribute("url", "");
		liObj.setAttribute("rel", "folder");
		liObj.setAttribute("mtype", "");
...
function startUpload(filename) {
	if ( filename && initialSelect )
	{
		...
		liObj.setAttribute("src", "http://dev.video.ettrema.com/skins/Big_Buck_Bunny_Trailer_480x270.png");
