 ...
 * @author brad
 */
public class ReportingApp implements MenuApplication, EventListener, LifecycleApplication, ChildPageApplication, AttachmentApplication {
    ...
    @Override
    public List<CommonResource> getAvailableAttachments(OrganisationFolder organisationFolder) {
        try {
            List<CommonResource> list = new ArrayList<>();
            ReportsHomeFolder reportsHomeFolder = (ReportsHomeFolder) organisationFolder.child(ReportsHomeFolder.HOME_NAME);
            appendReports(reportsHomeFolder, list);
            return list;
    ...
    private void appendReports(CollectionResource folder, List<CommonResource> list) throws NotAuthorizedException, BadRequestException {
        for (Resource r : folder.getChildren()) {
            if (r instanceof CollectionResource) {
                CollectionResource childCol = (CollectionResource) r;
                appendReports(childCol, list);
            } else {
                if (r instanceof IReportPage) {
                    IReportPage p = (IReportPage) r;
                    if (p.isAttachable()) {
                        list.add(p);
                    }
                }
