...
public class ProcessContext {
     ...
     * @param token
     * @param process
     * @param timerService - only needed if using time dependent states, otherwise
     * can be null
     */
    ...
    /**
     *  Scan for triggers on the current state. This is recursive, so if a transition
     * occurs it will scan again, and so forth. Because of the likelihood of infinite
     * recursion, this implements a maximum number of auto transitions - see MAX_TRANSITIONS
     */
    public boolean scan() {
        log.debug( "scan" );
        int cnt = 0;
